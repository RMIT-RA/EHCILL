...
void ServerImplement::update() 
{
	...
	//OPENNING NEW THREADS TO RECEIVE AND SEND DATA.
	//receiveFromClients();
	recv_Thread = std::thread(&ServerImplement::receiveFromClients, this);
	send_Thread = std::thread(&ServerImplement::sendToClients, this);
...
void ServerImplement::receiveFromClients()
{
    ...
    for(iter = network->sessions.begin(); iter != network->sessions.end(); iter++)
    {
        ...
        data_length = network->receiveData(iter->first, network_data);
	...
	printf("data_length from recv thread: %d\n", recv_Thread.get_id());//data_length);


	
}

void ServerImplement::sendToClients()
{

	//data_length = 10;
	printf("data_length from send thread: %d\n", send_Thread.get_id());//data_length);
